...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    inherit_prob = lambda parent_name: 1 - PROBS['mutation'] if parent_name in two_genes else 0.5 if parent_name in one_gene else PROBS['mutation']
    ...
    for person in people:
        ...
        if not mother and not father:
            genes_prob *= PROBS['gene'][num_genes]
        else:
            mother_pro = inherit_prob(mother)
            father_pro = inherit_prob(father)

            if num_genes == 2:
                genes_prob *= mother_pro * father_pro
            elif num_genes == 1:
                genes_prob *= (1 - mother_pro) * father_pro + (1 - father_pro) * mother_pro
            else:
                genes_prob *= (1 - mother_pro) * (1 - father_pro)

        genes_prob *= PROBS['trait'][num_genes][trait_person]

        probability *= genes_prob
