...
def buy():
    if request.method == "GET":
        ...
        if not symbol:
            return apology("Must fill the Symbol")

        stock = lookup(symbol.upper())

        if stock == None:
            return apology ("Stock not fund")
        ...
        if shares < 0:
            return apology ("Share number incorrect")
        ...
        cash_up = user_cash - transaction_value
        db.execute("UPDATE users SET cash = ? WHERE id = ?", cash_up, user_id)
...
def sell():
    ...
    if request.method == "GET":
        ...
        user_shares_updt = user_shares[0]["shares"]

        if shares > user_shares_updt:
...
def usermenu():
    ...
    if request.method == "GET":
        ...
        user = db.execute("UPDATE users SET hash = :hash WHERE id = :id", hash=hash,  id=user)
